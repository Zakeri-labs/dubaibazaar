    ...
    <>
      ...
      <AnimatePresence>
        {open && (
          <>
            <motion.div
              className="fixed inset-0 bg-brand-ink/60 backdrop-blur-md z-[9998]"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={() => setOpen(false)}
            />
            <motion.aside
              className="fixed top-0 right-0 bottom-0 w-full bg-white z-[9999] p-8 flex flex-col shadow-2xl"
